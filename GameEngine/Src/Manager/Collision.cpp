...
void Manager::Collision::detectCollision(
    ...
    std::cout << "Collision::detectCollision between " << collider.size()
              << " entities" << std::endl;
    ...
    for (auto const &current: entityList1) {
        for (auto const &other: entityList2) {
            if (current->getId() == other->getId()
                ...
                continue;
            if (this->entityCollision(current, other)
                || this->entityCollision(other, current)) {
                std::cout << "Collision match : " << current->getEntityType()
                          << " - " << other->getEntityType() << std::endl;
...
bool Manager::Collision::entityCollision(const AEntityPtr &entity1,
                                         const AEntityPtr &entity2) {
