...
AMonster::AMonster(const sf::Vector2f &position, ACore &entryPoint,
    ...
    this->amplitude =
        (_amplitude < 0.2) ? 0.2 : (_amplitude > 10) ? 10 : _amplitude;
    this->amplitudeSpeed =
        _amplitudeSpeed < 0.01
            ? 0.01
            : _amplitudeSpeed > 0.05 ? 0.05 : _amplitudeSpeed;
    this->scale = _scale;
...
void AMonster::update() {
    ...
    this->position.y =
        this->originalY + std::cos(this->counter) * 10 * this->amplitude;
    if (std::rand() % 1000 < 5)
        entryPoint.feedEntity(std::make_shared<Bullet::Monster>(
            this->position, this->entryPoint));
    if (this->position.x < -110 || this->position.x > 110 ||
        this->position.y < -130 || this->position.y > 130) {
        this->position.x = 109;
...
AEntityPtr AMonster::createMonsterFromPacket(ACore &core, sf::Packet packet) {
    ...
    packet >> id >> pos.x >> pos.y >> speed >> amplitude >> amplitudeSpeed
        >> scale >> entity;
